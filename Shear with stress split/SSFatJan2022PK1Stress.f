...
C                                                          C   
C   Author: J.Reinoso,  Pavan Kumar Asur Vijaya kumar                  C
...
!-------------------------------- 
        ...
        REAL*8 Weas(6,nEAS) !Weas matri that computes T0(6x6)*Meas(6x30 as maximum)
...
!-------------------------------------------------------
       ...
       !xn(:,:) = 0.d0 !previous converged state	  
	    ...
	    xref(:,:) = coords(:,:)  ! Working fine (Xref now has coordniates)
...
!                !Maximum phase field
     ...
     *     gkovr,gkonr,gmkovr,gmkonr,detr)


           call ks8metShellBody10(xcur,shapef,dshape,xi,nnode,ndim,
     *     gkovc,gkonc,gmkovc,gmkonc,detc)
...
!     update crack length
       coordx=0.d0
       if (kPhinterp.ge.0.95d0) then
        do ii=1,nnode
         coordx=coordx+shapef(ii)*coords(1,ii)
        enddo
...
c
      ...
      !!!write(7,*) "Before first multiplication in triple"
      call kmatrixmultiS10(Aux,c1,d2,C,c1,c2,Trd,d1,d2)
       !!!write(7,*) "Before second multiplication in triple"  
      call kmatrixmultiS10(Ares,b1,d2,B,b1,c1,Aux,c1,d2)
      !!!write(7,*) "After second multiplication in triple"  
...
c     Green-Lagrange strain tensor  in vector form
	  ...
	  !CALL KMATRIXMULTS10TM(Strain_cart,6,1,transcart,6,6,Ev,6,1)
...
! MATERIAL KIRCHHOFF-SAINT-Venant
     ...
     *   c2*(gmkonr(i,k)*gmkonr(j,l) + gmkonr(i,l)*gmkonr(k,j))
      ...
      El(3) =Ev(3)*2.d0
      El(5) =Ev(5)*2.d0
	  ! Compute the transformation into cartesian for strains  
	  !CALL KMATRIXMULTS10TM(Strain_norCart,6,1,transcart,6,6,El,6,1) 
	 ...
	 ! Since CMAT and strains are already in Cartersian. the stress is also in caretsian 
      ...
      enddo
			   ...
			   ! Find the strian in cartisian co-ordinates
		!call	s8_kov_cuca(EGL,EGL_cart,gkonr)
		...
		call s8_kon_cuca(stressL,stress_cart,gkovr) !New routine by Jose
		!compute the PK2 in Cartesian frame 
		
		      !call kCompTransposeS10(DefGradInv,3,3,DefGradInvT)
		      call kCompTransposeS10(DefGrad,3,3,DefGradT)
      !kCompTranspose(A,m,n,AT)  A(n,m) , At(m,n)
		
			!write(7,*)'DefGrad', DefGrad
			!write(7,*)'DefGradT', DefGradT
!		      call kmatrixmultiS10(EGL_cart1,3,3,DefGradInvT,
!     * 3,3,EGL_cart,3,3)
!		EGL_cart1(:,:)=MATMUL(DefGradInvT(:,:),EGL_cart(:,:))	
		!write(7,*)'DefGradInv ', DefGradInv
		!write(7,*)'DefGradInvT ', DefGradInvT 
		
!		  call kmatrixmultiS10(Euler_cart,3,3,EGL_cart1,
 !    * 3,3,DefGradInv,3,3)
...
!            cauchy  = matmul(DefGrad,	 

			cauchy(:,:)= cauchy(:,:)/DetF
	
	
!		Euler_cart(:,:)=MATMUL(EGL_cart1(:,:),DefGradInv(:,:))
		
!		write(7,*)'cauchy1 ', cauchy1 !
!		write(7,*)'cauchy ', cauchy 
		!write(7,*)'EGL_cart1   intermediate', EGL_cart1 
		
		
		!write(7,*)'Euler_cart   intermediate', Euler_cart 
		

      !kmatrixmulti(A,a1,a2,B,b1,b2,C,c1,c2)  A = B*C
		!Euler_cart(:,:)=Euler_cart(:,:)*DetF
!		call kmatrixtriplS10(Euler_cart,3,3,DefGradInvT,3,3,
!     * EGL_cart,3,3,DefGradInv,3,3)  
	 
			!write(7,*)'DefGradInv   intermediate', DefGradInvT 
			!write(7,*)'DefGradInvT   intermediate', DefGradInvT 
		
		
!c        This subroutine computing 
!c        A = B*C*D
		
		 !CALL SPRINC(Euler_cart,EGL_princ,2,3,3) ! Compute principle strains
		 !CALL SPRINC(Euler_cart,EGL_princ,2,3,3) ! Compute principle strains
		 !write(7,*)'DefGrad', DefGrad
		 !write(7,*)'DefGradInv', DefGradInv
		 !write(7,*)'Euler_cart', Euler_cart
		!!write(7,*)'EGL', EGL  
		!write(7,*)'EGL_cart', EGL_cart  
		!write(7,*)'EGL_princ', EGL_princ  
		
		!!
		!TraceE=EGL_princ(1)+EGL_princ(2)+EGL_princ(3)
		
		!Edeviotoric(1:3)=EGL_princ(1:3)-TraceE/3.0d0
		
		!Norm=Edeviotoric(1)**2.d0+Edeviotoric(2)**2.d0+Edeviotoric(3)**2.d0
		
		!TraceE_pos=0.5d0*(TraceE+abs(TraceE))
		!TraceE_neg=0.5d0*(TraceE-abs(TraceE))

		
		!kFreeEner_pos=0.5d0*bk*TraceE_pos**2.d0+eg*Norm
		!kFreeEner_neg=0.5d0*bk*TraceE_neg**2.d0
		
		!-----------------------For stresses
!for streses
			CALL SPRINC(stress_cart,stress_princ,2,3,3) ! Compute principle stress
		
		sig_pos= 0.5d0*(abs(stress_princ)+stress_princ)
			
			!write(7,*)'sig_pos', sig_pos
				
		tr_sig=stress_princ(1)+stress_princ(2)+stress_princ(3)
		tr_sig_pos=0.5d0*(abs(tr_sig)+tr_sig)   
                !print*, 'tr_sig_pos', tr_sig_pos				
!		write(7,*)'stress_princ', stress_princ
		!write(7,*)'tr_sig_pos', tr_sig_pos
     ...
     * 0.5d0*enu*(tr_sig_pos**2)/emod 
		...
		!write(7,*)'TraceE', TraceE
		!write(7,*)'Edeviotoric', Edeviotoric
		!write(7,*)'Norm', Norm
		!write(7,*)'TraceE_pos', TraceE_pos
		!write(7,*)'TraceE_neg', TraceE_neg
		!write(7,*)'kFreeEner_pos', kFreeEner_pos
		!write(7,*)'kFreeEner_neg', kFreeEner_neg
