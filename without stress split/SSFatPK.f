...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      integer nelem
	  INTEGER,PARAMETER :: allelem = 2784
      save
      end module
!-----------------------------------------------------------------------
!------------------------------------------------------------------------                                    
...
!      INTEGER,PARAMETER :: nnode = 8 !number of element nodes
      ...
      INTEGER,PARAMETER :: PFTol  = 0.5d0 !dimension
      ...
      !--------------------------------
      REAL*8 emod,enu,matflag,KFreeEner
      !--------------------------------
...
!                !Maximum phase field
     ...
     *     gkovc,gkonc,gmkovc,gmkonc,detc)
      ...
      !Constitutive Law
      call kMatLawS10(Cmat,gmkovc,gmkovr,gmkonr,
     *    emod,enu,stress,strain, ntens,KFreeEner)
...
!---------------------------------------------------------    
      ...
      subroutine kMatLawS10(Cmat,gmkovc,gmkovr,gmkonr,
     *  emod,enu,stress,strain, ntens,kFreeEner)
        
        
      INTEGER ntens, Nota(3,3)

      REAL*8 Cmat(ntens,ntens), transcart(6,6)
      REAL*8 stress(ntens), Cmat_Cart(6,6), emod,enu,kFreeEner
      REAL*8 strain(ntens), gmkovc(3,3), gmkovr(3,3),gmkonr(3,3)
      REAL*8 Ev(ntens), El(ntens), EGL(3,3), C(3,3,3,3)
      REAL*8 Strain_cart(ntens), Strain_norCart(ntens), stress_cart(ntens)
